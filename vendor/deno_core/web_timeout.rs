...
impl MutableSleep {
  ...
  fn change(self: &Box<Self>, instant: Instant) {
    // NeoRender patch: catch potential UnsafeCell data race instead of panicking.
    // The original code can panic with "already borrowed" when the sleep future
    // is polled concurrently during nested event loop execution.
    let pin = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| unsafe {
      ...
      *self.sleep.get() = Some(tokio::time::sleep_until(instant));
      ...
      Ok(pin) => pin,
      Err(_) => {
        // Data race detected — skip this timer change silently.
        // The timer will fire on the next event loop iteration instead.
        return;
      }
    };
