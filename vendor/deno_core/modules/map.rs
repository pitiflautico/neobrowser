...
impl ModuleMap {
  ...
  pub fn mod_evaluate(
    ...
    // NeoRender patch: don't panic on "Module already evaluated".
    // Instead, return an already-resolved future with Ok(()).
    // This happens when a module is loaded as a transitive dependency
    // during another module's event loop, and then explicitly loaded again.
    // NeoRender patch: if module already evaluated, just proceed.
    // The original code panics with "Module already evaluated".
    // We silently skip re-evaluation — the module already ran its side effects.
    if status == v8::ModuleStatus::Evaluated {
      status = v8::ModuleStatus::Instantiated; // trick: pretend it's instantiated
      // V8 will just return the existing evaluation result
    }
    assert_eq!(
      status,
