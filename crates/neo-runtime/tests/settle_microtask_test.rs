...
fn settle_t1_event_loop_timeout_with_pending() {
    ...
    assert!(test_microtask(&mut rt, &tokio_rt, "t1-before"));
    ...
    test_microtask(&mut rt, &tokio_rt, "t1-after-timeout-settle");
}
...
fn settle_t2_watchdog_terminate() {
    ...
    let mut rt = make_runtime();
    assert!(test_microtask(&mut rt, &tokio_rt, "t2-before"));
    ...
    let ok = test_microtask(&mut rt, &tokio_rt, "t2-after-watchdog");
    if !ok {
...
fn settle_t3_timeout_only() {
    ...
    let mut rt = make_runtime();
    assert!(test_microtask(&mut rt, &tokio_rt, "t3-before"));
    ...
    let ok = test_microtask(&mut rt, &tokio_rt, "t3-after-timeout-only");
    if !ok {
...
fn settle_t4_terminate_no_event_loop() {
    ...
    let mut rt = make_runtime();
    assert!(test_microtask(&mut rt, &tokio_rt, "t4-before"));
    ...
    let ok = test_microtask(&mut rt, &tokio_rt, "t4-after-terminate-no-loop");
    if !ok {
