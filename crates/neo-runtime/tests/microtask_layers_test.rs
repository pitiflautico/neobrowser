...
//! NOTE: deno_core 0.393 requires explicit event loop pump for microtask drain.
//!
...
fn test_microtask(rt: &mut DenoJsRuntime, tokio_rt: &tokio::runtime::Runtime, label: &str, var: &str) -> bool {
    let code = format!(
...
fn make_tokio() -> tokio::runtime::Runtime {
    tokio::runtime::Builder::new_current_thread()
        .enable_all().build().unwrap()
}

fn load_js_stack(rt: &mut DenoJsRuntime, files: &[&str]) {
...
#[test]
fn layer_0_bare() {
    let tokio_rt = make_tokio();
    let _guard = tokio_rt.enter();
    let mut rt = DenoJsRuntime::new(RuntimeOptions::default());
    assert!(test_microtask(&mut rt, &tokio_rt, "L0-bare", "__l0"));
}
...
fn layer_1_tokio() {
    ...
    let _guard = tokio_rt.enter();
    let mut rt = DenoJsRuntime::new(RuntimeOptions::default());
    assert!(test_microtask(&mut rt, &tokio_rt, "L1-tokio", "__l1"));
}
...
fn layer_2_extension() {
    ...
    assert!(test_microtask(&mut rt, &tokio_rt, "L2-ext", "__l2"));
}
...
fn layer_3_drain_fn() {
    ...
    assert!(test_microtask(&mut rt, &tokio_rt, "L3-drain", "__l3"));
}
...
fn layer_4_happydom() {
    ...
    load_js_stack(&mut rt, &["happydom.bundle.js"]);
    assert!(test_microtask(&mut rt, &tokio_rt, "L4-happydom", "__l4"));
}
...
fn layer_5_bootstrap() {
    ...
    load_js_stack(&mut rt, &["happydom.bundle.js", "bootstrap.js"]);
    assert!(test_microtask(&mut rt, &tokio_rt, "L5-bootstrap", "__l5"));
}
...
fn layer_6_shim() {
    ...
    load_js_stack(&mut rt, &["happydom.bundle.js", "bootstrap.js", "browser_shim.js"]);
    assert!(test_microtask(&mut rt, &tokio_rt, "L6-shim", "__l6"));
}
...
fn layer_7_sentinel() {
    ...
    load_js_stack(&mut rt, &["happydom.bundle.js", "bootstrap.js", "browser_shim.js", "sentinel.js"]);
    assert!(test_microtask(&mut rt, &tokio_rt, "L7-sentinel", "__l7"));
}
...
fn layer_8_turbostream() {
    ...
    load_js_stack(&mut rt, &["happydom.bundle.js", "bootstrap.js", "browser_shim.js", "sentinel.js", "turbo-stream.bundle.js"]);
    assert!(test_microtask(&mut rt, &tokio_rt, "L8-turbo", "__l8"));
}
...
fn layer_9_html_set() {
    ...
    assert!(test_microtask(&mut rt, &tokio_rt, "L9-html", "__l9"));
}
...
fn layer_10_after_settle() {
    ...
    assert!(test_microtask(&mut rt, &tokio_rt, "L10-settle", "__l10"));
}
