...
use crate::{
    CacheDecision, CookieStore, HttpCache, HttpClient, HttpError, HttpRequest, WebStorage,
};
use neo_types::{Cookie, HttpResponse};
use std::collections::HashMap;
use std::sync::Mutex;

/// Mock HTTP client for testing without real network I/O.
#[derive(Debug)]
...
/// Builder for adding mock response rules to a [`MockHttpClient`].
#[derive(Debug)]
...
impl MockHttpClient {
    /// Create a new mock HTTP client with no rules or blocks.
    pub fn new() -> Self {
        Self {
    ...
    /// Register a URL substring pattern to match against incoming requests.
    ...
    /// Add a URL substring pattern to the block list.
    pub fn block_pattern(&self, pattern: &str) {
        self.blocked
            .lock()
    ...
    pub fn is_blocked(&self, url: &str) -> bool {
        let blocked = self.blocked.lock().expect("lock poisoned");
        blocked.iter().any(|p| url.contains(p.as_str()))
    }
}
...
impl<'a> MockRuleBuilder<'a> {
    /// Set the response to return when the URL pattern matches.
...
impl HttpClient for MockHttpClient {
    fn request(&self, req: &HttpRequest) -> Result<HttpResponse, HttpError> {
        self.recorded
            ...
            .expect("lock poisoned")
            .push(req.clone());
        // Blocked URLs return 200 with empty body (simulates resource blocking)
        if self.is_blocked(&req.url) {
...
/// In-memory cookie store for testing without persistence.
...
impl InMemoryCookieStore {
    /// Create a new empty in-memory cookie store.
    pub fn new() -> Self {
        Self::default()
    }
}
impl CookieStore for InMemoryCookieStore {
    ...
    fn import(&self, cookies: &[Cookie]) {
        ...
        for c in cookies {
            store.insert(format!("{}:{}", c.domain, c.name), c.clone());
        }
    }
...
/// No-op cache that never stores or returns cached responses.
#[derive(Debug, Default)]
...
/// In-memory web storage for testing localStorage/sessionStorage.
#[derive(Debug, Default)]
pub struct InMemoryWebStorage {
    data: Mutex<HashMap<String, HashMap<String, String>>>,
}

impl InMemoryWebStorage {
    /// Create a new empty in-memory web storage.
    pub fn new() -> Self {
        Self::default()
    }
}
impl WebStorage for InMemoryWebStorage {
    ...
    fn remove(&self, origin: &str, key: &str) {
        if let Some(m) = self.data.lock().expect("lock poisoned").get_mut(origin) {
            m.remove(key);
        }
    }
    fn clear(&self, origin: &str) {
        self.data.lock().expect("lock poisoned").remove(origin);
    }
