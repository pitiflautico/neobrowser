...
class Session:
    ...
    def __init__(self, profile_name: str, visible: bool = False) -> None:
    ...
    def ensure(self) -> ChromeProcess:
        ...
        with self._lock:
            ...
            try:
                from neobrowser.cookie_sync import pre_launch_sync
            ...
            chrome = (ChromeProcess.launch_visible(self._profile_dir)
                      if self._visible
                      else ChromeProcess.launch(self._profile_dir))
    ...
    def open_tab(self) -> ChromeTab:
        ...
        try:
            from neobrowser.cookie_sync import post_launch_restore
...
class AttachedSession(Session):
    ...
    def open_tab(self) -> ChromeTab:
        """Open a new tab in the attached Chrome at self._port, with cookies injected."""
        tab = ChromeTab.open(self._port)
        try:
            from neobrowser.cookie_sync import inject_from_real_chrome
            injected = inject_from_real_chrome(tab, self.profile_name)
            if injected:
                import logging
                logging.getLogger(__name__).info(
                    "AttachedSession: injected %d cookies into tab (port=%d)",
                    injected, self._port,
                )
        except Exception as exc:
            import logging
            logging.getLogger(__name__).warning(
                "AttachedSession: cookie injection failed (non-fatal): %s", exc
            )
        return tab
