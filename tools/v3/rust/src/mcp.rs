...
fn tool_definitions() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            input_schema: serde_json::json!({
                ...
                "properties": {
                    "kind": {
                        "type": "string",
                        "enum": ["click", "type", "focus", "press", "scroll", "back", "forward", "reload", "eval", "hover", "select", "fill_form", "send_message", "drag", "upload", "clipboard_read", "clipboard_write", "mouse", "highlight", "get_info", "screenshot_annotated", "analyze_forms", "analyze_api", "extract_data", "list_frames", "switch_frame", "auto_frame", "main_frame"],
                        "description": "Action type. extract_data: auto-detect and extract structured data (JSON-LD, microdata, tables, products, articles, search results, pricing, navigation, metadata) — no site-specific code needed."
                    },
                    "target": {
                        "type": "string",
        ...
        ToolDef {
            ...
            description: "Run deterministic automation pipelines. Actions: goto, click, type, press, wait, eval, extract, send_message, wait_stable, save. Steps have retry, timeout, assert, store_as, on_fail. Use mode='neorender' for invisible execution (no Chrome).".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "pipeline": {
                    ...
                    "mode": {
                        "type": "string",
                        "enum": ["neorender", "chrome", "auto"],
                        "default": "auto",
                        "description": "Engine: neorender (V8+linkedom, invisible, no Chrome) | chrome (full browser, visible) | auto (neorender if active, else chrome)"
                    }
                }
            }),
        },
        ...
        ToolDef {
            ...
            description: "Web search optimized for AI: returns only [{title, url, snippet}] — minimal tokens. Uses DuckDuckGo HTML via Ghost (rquest, no Chrome). With deep=true, also fetches top 3 pages and extracts key content server-side.".into(),
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "query": { "type": "string", "description": "Search query" },
                    "num": { "type": "integer", "default": 10, "description": "Number of results (max 30)" },
                    "region": { "type": "string", "default": "", "description": "Region code (e.g. 'us-en', 'es-es'). Empty for all regions." },
                    "deep": { "type": "boolean", "default": false, "description": "Fetch top 3 result pages and extract content (~1500 extra tokens)" },
                    "deep_num": { "type": "integer", "default": 3, "description": "Number of pages to deep-fetch (1-5, default 3)" },
                    "deep_chars": { "type": "integer", "default": 800, "description": "Max chars of extracted text per page (default 800)" }
                },
                "required": ["query"]
            }),
        },
...
async fn handle_tool(state: &mut McpState, name: &str, args: &Value) -> Result<Value, String> {
    match name {
        ...
        "browser_fetch" => handle_fetch(state, args).await,
        ...
        "browser_search" => handle_search(args).await,
            ...
            if need_new {
                ...
                if let Some(neo) = state.neo_session.as_mut() {
                    if let Some(cookies_file) = args["cookies_file"].as_str() {
                        neo.cookie_jar().load_from_file(cookies_file).ok();
                    }
                }
            }
...
async fn handle_act(state: &mut McpState, args: &Value) -> Result<Value, String> {
    ...
    if let Some(neo) = state.neo_session.as_mut() {
        match kind {
            ...
            "fill_form" => {
                let fields = fill_fields.as_ref().ok_or("Missing 'fields' for fill_form")?;
                let result = neo.fill_form(fields)?;
                let elapsed = act_t0.elapsed().as_millis() as u64;
                let filled = result["filled"].as_u64().unwrap_or(0);
                let total = result["total"].as_u64().unwrap_or(0);
                return Ok(serde_json::json!({
                    "ok": true,
                    "engine": "neosession",
                    "outcome": "succeeded",
                    "effect": format!("filled {}/{} fields", filled, total),
            ...
            "extract_data" => {
                let result = neo.eval("__neo_extract_structured()").map_err(|e| format!("{e}"))?;
                let elapsed = act_t0.elapsed().as_millis() as u64;
                return Ok(serde_json::json!({
                    "ok": true,
                    "engine": "neosession",
    ...
    let (outcome, effect) = match kind {
        ...
        "extract_data" => {
            let result = session.eval(EXTRACT_DATA_JS).await.map_err(|e| format!("{e}"))?;
            ("succeeded", result)
        }
        "list_frames" => {
...
async fn handle_fetch(state: &mut McpState, args: &Value) -> Result<Value, String> {
    let url = args["url"].as_str().ok_or("Missing 'url'")?;
    let method = args["method"].as_str().unwrap_or("GET");
    let body = args["body"].as_str();

    // Reuse or create persistent GhostBrowser (cookies persist across calls)
    if state.ghost.is_none() {
        state.ghost = Some(crate::ghost::GhostBrowser::new());
    }
    let ghost = state.ghost.as_mut().unwrap();
    ...
    Ok(serde_json::json!({
        ...
        "body": text,
        "engine": "rquest",
        "note": "Persistent HTTP with Chrome 136 TLS — cookies preserved across calls"
    }))
}
...
async fn handle_pipeline(state: &mut McpState, args: &Value) -> Result<Value, String> {
    ...
    let mode = args["mode"].as_str().unwrap_or("auto");
    let use_neo = match mode {
        "neorender" => true,
        "chrome" => false,
        _ /* auto */ => state.neo_session.is_some(), // Use NeoSession if already active
    };

    // Initialize the correct engine
    if use_neo {
        // NeoSession: create if needed (first goto will load the page)
        // Don't launch Chrome!
    } else {
        state.ensure_session().await?;
    }

    let engine_name = if use_neo { "neosession" } else { "chrome" };
    ...
    for (i, step) in pipeline.steps.iter().enumerate() {
        ...
        for attempt in 0..=step.max_retries {
            ...
            let result = if use_neo {
                // ─── NeoRender path ───
                pipeline_step_neo(state, step, &mut variables).await
            } else {
                // ─── Chrome path ───
                let session = state.session.as_mut().unwrap();
                pipeline_step_chrome(session, step, &mut variables).await
            };

            match result {
                Ok(detail) => {
                    // Check assertion if present (NeoRender: use eval, Chrome: use see_page)
                    if let Some(ref expected) = step.assert_text {
                        let page_text = if use_neo {
                            state.neo_session.as_mut()
                                .and_then(|n| n.export_text().ok())
                                .unwrap_or_default()
                        } else {
                            state.session.as_mut().unwrap().see_page().await.unwrap_or_default()
                        };
                        if page_text.contains(expected) {
        ...
        // Record trace if enabled (only for Chrome sessions)
        if state.trace.is_enabled() {
            let url = state.session.as_ref().map(|s| s.last_url.clone()).unwrap_or_default();
            state.trace.record(
                &step.action,
    ...
    let mut pr = serde_json::to_value(PipelineResult {
        name: pipeline.name,
    ...
    // Add engine info so caller knows which engine was used
    pr.as_object_mut().unwrap().insert("engine".into(), serde_json::Value::String(engine_name.into()));

    Ok(pr)
}

/// Execute a pipeline step using NeoSession (V8+linkedom, no Chrome).
async fn pipeline_step_neo(
    state: &mut McpState,
    step: &crate::runner::Step,
    variables: &mut std::collections::HashMap<String, String>,
) -> Result<String, String> {
    match step.action.as_str() {
        "goto" => {
            let url = substitute_vars(&step.target, variables);
            eprintln!("[PIPELINE:NEO] goto {}", &url[..url.len().min(80)]);
            // Parse domain — create NeoSession if needed or domain changed
            ...
            if need_new {
                eprintln!("[PIPELINE:NEO] Creating NeoSession for domain: {}", domain);
                state.neo_session = None;
                let ns = crate::neorender::session::NeoSession::new(None)
                    .map_err(|e| format!("NeoSession init failed: {e}"))?;
                state.neo_session = Some(ns);
                state.neo_session_domain = Some(domain);
                eprintln!("[PIPELINE:NEO] NeoSession created OK");
            }

            let neo = state.neo_session.as_mut().unwrap();
            neo.goto(&url).await.map_err(|e| format!("NeoSession goto failed: {e}"))?;
            eprintln!("[PIPELINE:NEO] goto OK");
            Ok("navigated".to_string())
        }
        "eval" | "extract" => {
            let neo = state.neo_session.as_mut().ok_or("No NeoSession — call goto first")?;
            let js = if step.action == "extract" && step.value.is_empty() {
                "document.body.innerText.substring(0, 4000)".to_string()
            } else {
                substitute_vars(&step.value, variables)
            };
            let result = neo.eval(&js).map_err(|e| format!("{e}"))?;
        ...
        "click" => {
            let neo = state.neo_session.as_mut().ok_or("No NeoSession")?;
            let target = substitute_vars(&step.target, variables);
            neo.click(&target).await?;
        ...
        "type" => {
            ...
            neo.type_text(&target, &text)?;
            Ok(format!("typed {} chars into '{}'", text.len(), target))
        }
        "select" => {
            let neo = state.neo_session.as_mut().ok_or("No NeoSession")?;
            let target = substitute_vars(&step.target, variables);
            let value = substitute_vars(&step.value, variables);
            neo.select(&target, &value)?;
            Ok(format!("selected '{}' in '{}'", value, target))
        }
        "wait" => {
        ...
        "save" => {
            ...
            let content = if step.value.is_empty() {
                let neo = state.neo_session.as_mut().ok_or("No NeoSession")?;
                neo.export_text().unwrap_or_default()
            } else {
            ...
            Ok(format!("saved {} bytes to {path}", content.len()))
        }
        "screenshot" => {
            Err("screenshot not supported in NeoRender mode (no visual rendering)".into())
        }
        "send_message" | "wait_stable" => {
            Err(format!("{} not yet supported in NeoRender mode", step.action))
        }
        _ => Err(format!("Unknown step action: {}", step.action)),
    }
}

/// Execute a pipeline step using Chrome CDP session.
async fn pipeline_step_chrome(
    ...
    match step.action.as_str() {
        ...
        "screenshot" => {
            let b64 = session.screenshot_base64().await.map_err(|e| format!("{e}"))?;
            if let Some(ref var_name) = step.store_as {
        ...
        "save" => {
            let path = substitute_vars(&step.target, variables);
            let content = if step.value.is_empty() {
                session.eval("document.body.innerText").await.unwrap_or_default()
            } else {
                let var_name = substitute_vars(&step.value, variables);
                variables.get(&var_name).cloned().unwrap_or_default()
            };
    ...
    for line_result in stdin.lock().lines() {
        ...
        let response = match request.method.as_str() {
            ...
            "tools/call" => {
                ...
                // Catch panics so a tool crash doesn't kill the MCP server
                let tool_result_outer = AssertUnwindSafe(
                    handle_tool(&mut state, tool_name, tool_args)
                ).catch_unwind().await;

                let tool_outcome = match tool_result_outer {
                    Ok(Ok(result)) => {
                    ...
                    Err(panic_err) => {
                        ...
                        eprintln!("[MCP] Tool '{}' panicked: {}", tool_name, panic_msg);
                        ToolResult {
                            content: vec![ToolContent {
                                content_type: "text".into(),
                                text: format!("Fatal error (recovered): {}", panic_msg),
                            }],
                            is_error: Some(true),
                        }
                    }
                ...
                JsonRpcResponse {
                    jsonrpc: "2.0".into(),
                    id: request.id,
                    result: Some(serde_json::to_value(tool_outcome)?),
                    error: None,
                }
            }
            _ => {
...
// ─── Structured data extraction JS (injected into Chrome via CDP eval) ───

const EXTRACT_DATA_JS: &str = include_str!("../js/extract_data_cdp.js");
...
// ─── browser_search handler (AI-optimized web search) ───

async fn handle_search(args: &Value) -> Result<Value, String> {
    let query = args["query"].as_str().ok_or("Missing 'query'")?;
    let num = args["num"].as_u64().unwrap_or(10).min(30) as usize;
    let region = args["region"].as_str().unwrap_or("");
    ...
    // Fetch with Ghost (rquest + Chrome 136 TLS)
    let mut ghost = crate::ghost::GhostBrowser::new();
    let resp = ghost.call_api_with_headers(&url, "GET", None, &[
        ("Accept".into(), "text/html,application/xhtml+xml".into()),
    ]).await?;

    let html = resp["data"].as_str().unwrap_or("");
    if html.is_empty() {
        return Ok(serde_json::json!({
            "ok": false,
    ...
    // DDG uses: div.result > div.result__body > h2.result__title > a.result__a
    //           + a.result__snippet (snippet text)
    //           + a.result__url (display URL)
    ...
    if deep && !results.is_empty() {
        let deep_num = args["deep_num"].as_u64().unwrap_or(3).min(5) as usize;
        let deep_chars = args["deep_chars"].as_u64().unwrap_or(800).min(2000) as usize;

        let urls_to_fetch: Vec<String> = results.iter()
            .take(deep_num)
        ...
        for page_url in urls_to_fetch {
            let max_chars = deep_chars;
            handles.push(tokio::spawn(async move {
                let mut g = crate::ghost::GhostBrowser::new();
                match g.call_api_with_headers(&page_url, "GET", None, &[
                    ("Accept".into(), "text/html,application/xhtml+xml,*/*".into()),
                ]).await {
                    Ok(resp) => {
                        let html = resp["data"].as_str().unwrap_or("");
                        if html.is_empty() {
                            return (page_url, String::new());
                        }
                        ...
                        match dom {
                            Ok(dom) => {
                                let text = crate::ghost::extract_text(&dom.document, 0);
                                // Clean: collapse whitespace, trim, truncate
                                let clean: String = text
                                    .split_whitespace()
                                    .collect::<Vec<_>>()
                                    .join(" ");
                                let truncated = if clean.len() > max_chars {
                                    format!("{}…", &clean[..clean.floor_char_boundary(max_chars)])
                                } else {
                                    clean
                                };
                                (page_url, truncated)
                            }
                            Err(_) => (page_url, String::new()),
                        }
                    }
                    Err(_) => (page_url, String::new()),
                }
        ...
        for handle in handles {
            if let Ok((url, content)) = handle.await {
                if !content.is_empty() {
                    // Find the matching result and add content
                    for r in results.iter_mut() {
                        if r["url"].as_str() == Some(&url) {
...
/// DDG HTML structure:
///   div.result > div.links_main.result__body >
///     h2.result__title > a.result__a [href, text=title]
///     a.result__snippet [text=snippet]
///     .result__extras__url > a.result__url [text=display_url]
fn extract_ddg_results(node: &markup5ever_rcdom::Handle, results: &mut Vec<Value>) {
    use markup5ever_rcdom::NodeData;

    let tag = crate::ghost::tag_name(node);
    let class = crate::ghost::get_attr(node, "class");
...
fn extract_ddg_result_fields(
    ...
    if tag == "a" && class.contains("result__a") {
        *title = crate::ghost::extract_text(node, 0).trim().to_string();
        let h = crate::ghost::get_attr(node, "href");
        if !h.is_empty() {
            *href = h;
        }
    }
    ...
    if tag == "a" && class.contains("result__snippet") {
        *snippet = crate::ghost::extract_text(node, 0).trim().to_string();
    }
