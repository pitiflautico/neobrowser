    ...
    fn load(
        ...
        // Check pre-fetched store first
        {
            ...
            if let Some(code) = store.scripts.get(&url) {
                ...
                // Transform 1: Promise.allSettled polyfill (deno_core 0.311 compat)
                if patched.contains("Promise.allSettled(") {
                ...
                // Transform 2: Defer CJS factory execution in large SPA bundles.
                // Pattern: `export default factoryFn()` where factoryFn() contains React.render()
                // and blocks V8 event loop with infinite scheduler ticks.
                // Rewrite to: export default and save factory for post-eval execution with timeout control.
                // Transform 2: Defer CJS factory in large SPA bundles.
                // Vite/Webpack bundles end with `export default factoryFn()` which calls React.render()
                // and blocks V8 forever with scheduler loops. We defer the factory call.
                if patched.len() > 500_000 {
                    // Look for pattern: );export default XXXX()\n or ;\nexport default XXXX()
                    // Can be at end of a very long line (minified) or on its own line
                    if let Some(pos) = patched.rfind("export default ") {
                        let after = &patched[pos + 15..]; // after "export default "
                        // Extract factory name: everything before "()" and before ";"
                        if let Some(paren_pos) = after.find("()") {
                            let factory_candidate = after[..paren_pos].trim();
                            // Validate: short alphanumeric identifier (e7t, $init, _app, etc.)
                            if !factory_candidate.is_empty()
                                && factory_candidate.len() < 20
                                && factory_candidate.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '$')
                            {
                                eprintln!("[NEORENDER:TRANSFORM] Deferring SPA factory: {} in {}", factory_candidate, short);
                                // Replace the export default with deferred version
                                let old_export = format!("export default {}()", factory_candidate);
                                // Also handle with semicolon and with trailing content
                                let old_with_semi = format!("export default {}();", factory_candidate);
                                let new_export = format!(
                                    "globalThis.__neo_deferred_factory = {}; export default null",
                                    factory_candidate
                                );
                                if patched.contains(&old_with_semi) {
                                    patched = patched.replace(&old_with_semi, &format!("{};", new_export));
                                } else {
                                    patched = patched.replace(&old_export, &new_export);
                                }
                            }
                        }
                    }
        ...
        ModuleLoadResponse::Async(Box::pin(async move {
            let client = crate::http_client::chrome136_quick(2)
...
/// `cookie_headers` is a domain -> "name=val; name2=val2" map.
pub fn create_runtime_with_html(
    html: &str,
    url: &str,
    cookie_headers: &std::collections::HashMap<String, String>,
    ...
    if !cookie_headers.is_empty() {
        let cookies_json = serde_json::to_string(&cookie_headers).unwrap_or_default();
        ...
        eprintln!("[NEORENDER] Injected cookies for {} domains", cookie_headers.len());
    }
    ...
    // 3f0b. DOM Events — 8-strategy element resolution, click/type/find for V8
    let events_js: String = include_str!("../../js/events.js").to_string();
    runtime.execute_script("<neorender:events>", events_js)
        .map_err(|e| format!("Events load error: {e}"))?;
    ...
    // 3h0. CSRF-aware form analysis and filling
    let forms_js: String = include_str!("../../js/forms.js").to_string();
    runtime.execute_script("<neorender:forms>", forms_js)
        .map_err(|e| format!("Forms load error: {e}"))?;
