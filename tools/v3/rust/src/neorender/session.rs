...
pub struct NeoSession {
    ...
    url: String,
    current_domain: String, // tracks domain for cross-domain V8 reset
...
fn should_skip_resource(url: &str) -> bool {
    ...
    // Scripts to skip — they don't contribute to page content
    const SKIP_PATTERNS: &[&str] = &[
        // Analytics & tracking
        "google-analytics.com", "googletagmanager.com", "gtag/js",
        "analytics.", "segment.com", "segment.io",
        "hotjar.com", "clarity.ms", "fullstory.com",
        "mixpanel.com", "amplitude.com", "heap.io",
        "tracking", "pixel",
        // Ads
        "googlesyndication.com", "doubleclick.net", "adservice.",
        "adsense", "adsbygoogle",
        "facebook.net/en_us/fbevents", "connect.facebook.net",
        // APM & monitoring
        "elastic-apm", "sentry.io", "bugsnag.com",
        "newrelic.com", "datadoghq.com", "rum-",
        // Chat widgets
        ...
        // Cookie consent (handled separately)
        "cookiebot.com", "onetrust.com", "trustarc.com",
        // Other
        "recaptcha", "hcaptcha", "neverbounce.com",
        "hubspot.com", "hubapi.com",
        "pusher.com", "ably.io",
    ];
    SKIP_PATTERNS.iter().any(|pat| lower.contains(pat))
}
...
impl NeoSession {
    ...
    pub fn new(cookies_file: Option<&str>) -> Result<Self, String> {
        ...
        let cookie_headers = cookie_jar.all_headers();
        let (mut runtime, store) = v8_runtime::create_runtime_with_html(
            empty_html, empty_url, &cookie_headers, None,
        )?;
    ...
    async fn navigate(&mut self, url: &str, method: &str, body: Option<&str>, content_type: Option<&str>) -> Result<PageResult, String> {
        ...
        // 0. Detect domain change and reset V8 DOM/globals to prevent cross-site pollution
        let new_domain = url::Url::parse(url).ok()
            .and_then(|u| u.host_str().map(|s| s.to_string()))
            .unwrap_or_default();

        let domain_changed = !self.current_domain.is_empty() && self.current_domain != new_domain;

        if domain_changed {
            eprintln!("[NEOSESSION] Domain change detected: {} → {}, resetting DOM", self.current_domain, new_domain);
            // Reset the DOM to a blank state
            let _ = self.eval_internal(r#"
                if (typeof parseHTML === 'function') {
                    var newDoc = parseHTML('<!DOCTYPE html><html><head></head><body></body></html>');
            ...
            "#);
            // Clear global variables that sites may have set
            let _ = self.eval_internal(r#"
                if (typeof __macrotaskQueue !== 'undefined') { __macrotaskQueue = []; }
                if (typeof __console_buffer !== 'undefined') { __console_buffer = []; }
                if (typeof __deltaFingerprints !== 'undefined') { __deltaFingerprints = {}; }
            "#);
        }
        ...
        // End inline timer phase — after lifecycle, setTimeout(fn,0) goes to macrotask queue
        // instead of running synchronously. This enables proper microtask/macrotask ordering.
        self.runtime.execute_script("<neosession:end_inline_phase>",
            "if (typeof __neo_end_inline_phase === 'function') __neo_end_inline_phase();".to_string()
        ).ok();
        ...
        //     Timeout at 15s to avoid hanging on pages with infinite timers.
        {
            ...
            loop {
                ...
                // Pump macrotask queue — execute one macrotask at a time.
                // Each eval_internal() call is a V8 microtask checkpoint: after the callback
                // runs, V8 drains all microtasks before returning to Rust. This gives proper
                // browser spec ordering: macrotask → drain microtasks → macrotask → ...
                {
                    let mut total_pumped = 0i64;
                    for _ in 0..20 { // Cap at 20 per settle-loop iteration
                        let ran = self.eval_internal(
                            "typeof __neo_pump_one_task === 'function' ? __neo_pump_one_task() : 0"
                        ).ok().and_then(|s| s.parse::<i64>().ok()).unwrap_or(0);
                        if ran == 0 { break; }
                        total_pumped += ran;
                    }
                    if total_pumped > 0 {
                        stable_count = 0; // Macrotasks ran — reset stability
                    }
                }
                ...
                // Check pending macrotasks
                let pending_tasks: i64 = self.eval_internal(
                    "typeof __neo_pending_tasks === 'function' ? __neo_pending_tasks() : 0"
                ).ok().and_then(|s| s.parse().ok()).unwrap_or(0);

                if node_count == last_node_count && pending_tasks == 0 {
                    stable_count += 1;
                    if stable_count >= stable_threshold {
                        eprintln!("[NEOSESSION] DOM stable at {} nodes, 0 pending tasks, after {:?}",
                            node_count, stability_start.elapsed());
                        break;
                    }
                } else if node_count == last_node_count {
                    // DOM stable but tasks pending — partial stability
                    stable_count += 1;
                    if stable_count >= stable_threshold {
                        eprintln!("[NEOSESSION] DOM stable at {} nodes ({} tasks still pending) after {:?}",
                            node_count, pending_tasks, stability_start.elapsed());
                        break;
                    }
                } else {
                    stable_count = 0;
        ...
        // 13a. Execute deferred SPA factory (React render) with timeout control.
        // Large SPA bundles have their factory deferred to avoid blocking module evaluation.
        // The factory (e.g. e7t()) calls React.render() which creates infinite scheduler loops.
        // We run it in a separate thread with a hard timeout — V8 gets N seconds then we move on.
        if let Ok(has_factory) = self.eval_internal("typeof globalThis.__neo_deferred_factory === 'function' ? 'yes' : 'no'") {
            if has_factory.trim_matches('"') == "yes" {
                eprintln!("[NEOSESSION] Executing deferred SPA factory...");

                // Strategy: execute factory and pump event loop with a hard wall-clock timeout.
                // React's scheduler creates infinite microtask/timer chains — we let it run
                // for up to 5s, then forcefully stop by not polling the event loop anymore.
                let factory_start = std::time::Instant::now();
                let factory_timeout = std::time::Duration::from_secs(5);

                // Execute the factory script (this is synchronous and may block if React
                // enters its scheduler loop). If it returns quickly, great. If not, the
                // tokio timeout will cancel it.
                // Execute factory in a dedicated thread with hard timeout.
                // React's scheduler + sync ops can block V8 indefinitely.
                // By using a thread with a wall-clock timeout, we get partial rendering.
                //
                // Strategy: run execute_script on the current thread but wrap in tokio timeout.
                // execute_script is sync but V8 ops (fetch, timer) yield to tokio via thread::spawn.
                // The timeout should cut after N seconds regardless of V8 state.
                //
                // Problem: execute_script is truly sync — tokio::timeout can't interrupt it.
                // Solution: just let it run for up to the timeout, then check what we got.
                //
                // Actually: We accept that the factory may block. We give it the timeout
                // and let run_event_loop drain whatever it can. If it times out,
                // we still get a partial DOM that React started building.

                let factory_js = "try { globalThis.__neo_deferred_factory() } catch(e) { globalThis.__neo_factory_error = String(e) }";

                // Use V8 isolate termination to enforce hard timeout (same pattern as script execution).
                // This is the ONLY way to interrupt a sync V8 execution — tokio timeout can't do it.
                let isolate_handle = self.runtime.v8_isolate().thread_safe_handle();
                let cancelled = std::sync::Arc::new(std::sync::atomic::AtomicBool::new(false));
                let cancelled_clone = cancelled.clone();
                let timeout_secs = factory_timeout.as_secs();

                let timer = std::thread::spawn(move || {
                    for _ in 0..(timeout_secs * 100) {  // check every 10ms
                    ...
                    if !cancelled_clone.load(std::sync::atomic::Ordering::Relaxed) {
                        eprintln!("[NEOSESSION] Factory TIMEOUT ({}s) — terminating V8 execution", timeout_secs);
                        isolate_handle.terminate_execution();
                    }
                });

                eprintln!("[NEOSESSION] Factory execute_script starting (terminate in {}s)...", timeout_secs);
                self.runtime.execute_script("<neosession:deferred_factory>", factory_js.to_string()).ok();
                eprintln!("[NEOSESSION] Factory execute_script returned");

                // Cancel the factory timer
                ...
                // Skip event loop pump — it hangs on pending promises from React's scheduler.
                // The factory already executed React.render() synchronously. Whatever React
                // committed to the DOM during the sync execution is what we get.

                let root_len = self.eval_internal("document.getElementById('root')?.innerHTML?.length || 0")
                    .unwrap_or_else(|_| "?".into());
                eprintln!("[NEOSESSION] Deferred factory done — root: {} chars ({:?})",
                    root_len, factory_start.elapsed());
            }
        }
        ...
        {
            // Detect Google consent by structure, not hardcoded titles (language-agnostic)
            let is_google_consent = final_url.contains("consent.google")
                || self.eval_internal(r#"
                    !!(document.querySelector('form[action*="consent.google"]')
                    || document.querySelector('form[action*="/save"]input[name="set_eom"]')
                    || (document.querySelector('input[name="set_eom"]') && document.querySelector('form[action*="/save"]')))
                "#).unwrap_or_default().contains("true");
        ...
        // 18. Thin page detection — consent cookies may have been set but the page
        //     was already rendered without them (e.g. Google search returns empty HTML
        //     on first visit without consent cookies). If the page has very few links
        //     and minimal text, retry once with the cookies now in the jar.
        let links_count = wom.as_ref()
            .and_then(|w| w["links"].as_array())
            .map(|a| a.len())
            .unwrap_or(0);
        let visible_text_len = text.len();
        let is_thin_page = links_count < 5 && visible_text_len < 500;
        if is_thin_page && html_len > 0 && self.cookie_jar.count() > 0 {
            eprintln!("[NEOSESSION] Thin page detected ({} links, {} text chars, {} html bytes) — retrying with current cookies",
                links_count, visible_text_len, html_len);
            return self.goto_consent_retry(url, start).await;
        }
    ...
    /// Wraps user code in IIFE to prevent scope pollution across eval calls.
    /// If code contains `return`, wraps in `(function(){ ... })()`.
    /// If code doesn't contain `return`, wraps the last expression as the return value.
    fn eval_internal(&mut self, js: &str) -> Result<String, String> {
        // Don't wrap simple global reads (used internally for __eval_result, etc.)
        let wrapped = if js.starts_with("globalThis.") || js.starts_with("__") {
            js.to_string()
        } else if js.contains("return ") || js.contains("return\n") {
            // User explicitly uses return — wrap in IIFE as-is
            format!("(function(){{ {} }})()", js)
        } else {
            // No return — auto-return the last expression
            // Split by semicolons/newlines, wrap last expression with return
            let trimmed = js.trim().trim_end_matches(';');
            if let Some(last_semi) = trimmed.rfind(';') {
                let (head, tail) = trimmed.split_at(last_semi + 1);
                format!("(function(){{ {}; return ({}); }})()", head.trim(), tail.trim())
            } else if let Some(last_nl) = trimmed.rfind('\n') {
                let (head, tail) = trimmed.split_at(last_nl + 1);
                let tail = tail.trim();
                // Only auto-return if last line looks like an expression (not a statement)
                if tail.starts_with("var ") || tail.starts_with("let ") || tail.starts_with("const ")
                    || tail.starts_with("if ") || tail.starts_with("for ") || tail.starts_with("while ")
                {
                    format!("(function(){{ {} }})()", trimmed)
                } else {
                    format!("(function(){{ {}; return ({}); }})()", head.trim(), tail)
                }
            } else {
                // Single expression — just wrap and return it
                format!("(function(){{ return ({}); }})()", trimmed)
            }
        };
        let result = self.runtime.execute_script("<neosession:eval>", wrapped)
