//! Real CDP transport — WebSocket and pipe connections to Chrome.
//!
...
// Make CdpError Send + Sync for the trait
unsafe impl Send for CdpError {}
unsafe impl Sync for CdpError {}
...
impl CdpSession {
    ...
    /// This is the internal method; the trait uses `send()`.
    ...
    /// Get a shareable reference to the ID counter.
    pub fn shared_id_counter(&self) -> Arc<AtomicU64> {
        self.next_id.clone()
    }

    /// Subscribe to a CDP event (internal method; trait uses on_event).
    pub async fn on_raw(&self, event: &str, callback: EventCallback) {
    ...
    /// Wait for a specific CDP event (one-shot, internal).
    ...
    // Legacy API compatibility — these map to the old public names
    // so engine.rs doesn't need changes yet.

    /// Send a CDP command (legacy API).
    pub async fn send(
        &self,
        method: &str,
        params: Option<Value>,
    ...
    pub async fn send_to(
        ...
        self.send_to_raw(session_id, method, params).await
    }
    ...
    /// Wait for event (legacy API).
    pub async fn wait_for(
        &self,
        event: &str,
        timeout_ms: u64,
    ) -> Result<Value, Box<dyn std::error::Error>> {
        self.wait_for_raw(event, timeout_ms).await
    }
}
