...
// Console capture — buffer for browser_trace, never stdout.
// In production bootstrap.js sets this up with stderr routing;
// this fallback covers dom.js-only usage.
const __neo_console_buffer = globalThis.__neo_console_buffer || [];
const _capture = (level) => (...a) => {
    const strs = a.map(String);
    __neo_console_buffer.push({level, args: strs, ts: Date.now()});
    if (__neo_console_buffer.length > 100) __neo_console_buffer.shift();
};
const _noop = () => {};
const console = {
    log: _capture('log'), warn: _capture('warn'), error: _capture('error'),
    info: _capture('info'), debug: _noop, trace: _noop,
    dir: _noop, table: _noop,
    group: _noop, groupEnd: _noop,
    time: _noop, timeEnd: _noop,
    assert: _noop, clear: _noop, count: _noop, countReset: _noop,
};
if (!globalThis.__neo_console_buffer) globalThis.__neo_console_buffer = __neo_console_buffer;
if (!globalThis.__neo_get_console) {
    globalThis.__neo_get_console = () => {
        const buf = JSON.stringify(__neo_console_buffer);
        __neo_console_buffer.length = 0;
        return buf;
    };
}
