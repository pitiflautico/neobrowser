...
impl GhostBrowser {
    ...
    /// Navigate with JS execution — fetches HTML, downloads scripts, runs them in Ghost DOM.
    /// Returns a richer page map than plain goto().
    pub async fn goto_js(&mut self, url: &str) -> Result<PageResult, String> {
        // 1. Fetch HTML
        let headers = self.chrome_headers(url);
        let resp = self.client.get(url)
            .headers(headers)
        ...
        let (inline_scripts, external_urls) = crate::ghost_dom::extract_scripts(&html);

        // 3. Fetch external scripts (parallel, with timeout)
        ...
        for script_url in &external_urls {
            let full_url = if script_url.starts_with("http") {
                ...
                base.join(script_url).map(|u| u.to_string()).unwrap_or_default()
            } else {
                continue;
            };
            if full_url.is_empty() { continue; }

            // Fetch with timeout — don't let one slow script block everything
            let client = self.client.clone();
            match tokio::time::timeout(
                std::time::Duration::from_secs(5),
                client.get(&full_url).send()
            ).await {
                Ok(Ok(resp)) => {
                    if let Ok(text) = resp.text().await {
                        all_scripts.push(text);
                    }
                }
                _ => {
                    eprintln!("[GHOST] Skip slow/failed script: {full_url}");
                }
            }
        }
        // Add inline scripts after external (typical browser order)
        all_scripts.extend(inline_scripts);

        // 4. Execute in Ghost DOM
        let dom_json = crate::ghost_dom::execute_page(&html, &all_scripts, &final_url)
            .unwrap_or_else(|e| {
                eprintln!("[GHOST] JS execution error (non-fatal): {e}");
                "{}".to_string()
            });

        // 5. Parse the DOM JSON into our PageResult format
        let page = self.dom_json_to_page(&dom_json, &final_url, status, html_len);

        self.last_url = final_url;
        self.last_html = html;
        self.history.push(self.last_url.clone());

        Ok(page)
    }

    /// Convert Ghost DOM JSON tree into PageResult
    fn dom_json_to_page(&self, json: &str, url: &str, status: u16, html_len: usize) -> PageResult {
        let tree: serde_json::Value = serde_json::from_str(json).unwrap_or_default();
        ...
        fn walk(
            ...
            title: &mut String, text: &mut String,
            links: &mut Vec<Link>, forms: &mut Vec<Form>,
            inputs: &mut Vec<Input>, buttons: &mut Vec<Button>,
            base_url: &str,
        ) {
            match node["type"].as_str() {
                ...
                Some("element") => {
                    ...
                    // Links
                    if tag == "a" {
                        if let Some(href) = attrs["href"].as_str() {
                    ...
                    if tag == "input" {
                        inputs.push(Input {
                            ...
                            input_type: attrs["type"].as_str().unwrap_or("").to_string(),
                            placeholder: attrs["placeholder"].as_str().unwrap_or("").to_string(),
                            value: attrs["value"].as_str().unwrap_or("").to_string(),
                            id: attrs["id"].as_str().unwrap_or("").to_string(),
                        });
                    }

                    // Buttons
                    if tag == "button" {
                        let btn_text = collect_text(node);
                        buttons.push(Button {
                            text: btn_text,
                            button_type: attrs["type"].as_str().unwrap_or("").to_string(),
                            name: attrs["name"].as_str().unwrap_or("").to_string(),
                        });
                    }
                    ...
                    if tag == "form" {
                        ...
                        collect_form_fields_json(node, &mut fields);
                        forms.push(Form {
                            action: attrs["action"].as_str().unwrap_or("").to_string(),
                            method: attrs["method"].as_str().unwrap_or("GET").to_uppercase(),
                            fields,
                        });
                    }

                    // Skip invisible tags for text
                    if !matches!(tag, "script" | "style" | "noscript" | "svg") {
                        if let Some(children) = node["children"].as_array() {
                            for child in children {
                                walk(child, title, text, links, forms, inputs, buttons, base_url);
                            }
                        }
                    }

                    // Block element newlines
                    if matches!(tag, "p" | "div" | "h1" | "h2" | "h3" | "h4" | "h5" | "h6" | "li" | "tr" | "br") {
                        text.push('\n');
                    }
                }
                _ => {}
        ...
        fn collect_text(node: &serde_json::Value) -> String {
            ...
            if let Some(text) = node["text"].as_str() { t.push_str(text); }
            if let Some(children) = node["children"].as_array() {
                for child in children {
                    t.push_str(&collect_text(child));
                }
            }
            t.trim().to_string()
        }

        fn collect_form_fields_json(node: &serde_json::Value, fields: &mut Vec<FormField>) {
                ...
                if matches!(tag, "input" | "select" | "textarea") {
                    ...
                    if !name.is_empty() {
                        fields.push(FormField {
                            name,
                            field_type: node["attrs"]["type"].as_str().unwrap_or("").to_string(),
                            value: node["attrs"]["value"].as_str().unwrap_or("").to_string(),
                        });
                    }
                }
            }
            if let Some(children) = node["children"].as_array() {
                for child in children {
                    collect_form_fields_json(child, fields);
                }
            }
        }

        walk(&tree, &mut title, &mut text, &mut links, &mut forms, &mut inputs, &mut buttons, url);
    ...
    pub async fn call_api(&mut self, url: &str, method: &str, body: Option<&str>) -> Result<serde_json::Value, String> {
        ...
        let resp = match method.to_uppercase().as_str() {
            "POST" => {
                ...
                self.client.post(url).headers(headers).body(b.to_string()).send().await
            }
            "PUT" => {
                let b = body.unwrap_or("{}");
                self.client.put(url).headers(headers).body(b.to_string()).send().await
            }
            "DELETE" => {
            ...
            "PATCH" => {
                let b = body.unwrap_or("{}");
                self.client.patch(url).headers(headers).body(b.to_string()).send().await
            }
            _ => {
...
fn get_attr(node: &Handle, name: &str) -> String {
    match &node.data {
...
fn tag_name(node: &Handle) -> String {
    match &node.data {
...
fn extract_text(node: &Handle, depth: usize) -> String {
