...
def make_browser(profile: str = "default") -> tuple["Browser", MagicMock, MagicMock, MagicMock]:
    ...
    with (
        patch("neobrowser.browser.Session", return_value=mock_session),
        patch("neobrowser.browser.TabPool", return_value=mock_pool),
        patch("neobrowser.browser.PageAnalyzer", return_value=mock_analyzer),
    ):
...
def test_browser_constructs_components():
    with (
        patch("neobrowser.browser.Session") as MockSession,
        patch("neobrowser.browser.TabPool") as MockPool,
        patch("neobrowser.browser.PageAnalyzer") as MockAnalyzer,
    ):
...
def test_context_manager_calls_close():
    with (
        patch("neobrowser.browser.Session"),
        patch("neobrowser.browser.TabPool") as MockPool,
        patch("neobrowser.browser.PageAnalyzer"),
    ):
        MockPool.return_value.stats.return_value = {"total": 0, "idle": 0, "in_use": 0}
        with Browser() as b:
            mock_pool = b._pool
