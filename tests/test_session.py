...
from neobrowser.session import Session, _validate_profile_name, CHROME_READY_TIMEOUT
...
class TestSessionEnsure:
    ...
    @patch("neobrowser.session.wait_for_chrome", return_value=False)
    @patch("neobrowser.session.ChromeProcess.launch")
    ...
    def test_profile_dir_uses_profile_name(self, mock_launch, mock_wait):
        ...
        from neobrowser.session import PROFILES_BASE
        chrome = _make_healthy_chrome()
        mock_launch.return_value = chrome
...
class TestSessionSecurityFixes:
    ...
    def test_sibling_directory_bypass_blocked(self):
        ...
        from neobrowser.chrome_process import ChromeProcess, PROFILES_BASE
        from pathlib import Path
