...
async def main():
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            # Use keyboard shortcut 'n' to open compose (reliable on X home)
            await call(session, "js", {"code": """
                document.dispatchEvent(new KeyboardEvent('keydown', {key: 'n', code: 'KeyN', keyCode: 78, bubbles: true}));
                document.dispatchEvent(new KeyboardEvent('keyup', {key: 'n', code: 'KeyN', keyCode: 78, bubbles: true}));
                return 'PRESSED_N';
            """})
            await asyncio.sleep(3)
            ...
            # Submit with Ctrl+Enter (reliable on X composer)
            await call(session, "js", {"code": """
